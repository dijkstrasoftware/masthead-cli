defmodule MastheadCli.Renderer.V1 do
  @moduledoc """
  Render contract v1 — faithful copy of `Masthead.Themes.Renderer.V1`, for
  themes whose manifest declares `"render_version": "v1"`.

  What changed from `MastheadCli.Renderer.Beta`:

    * per-page settings are declared as `page_options` and reach templates as
      `page.page_options.<key>`
  ...
  defp render(theme, site, target, assigns, target_assigns) do
    ...
    base_context = %{
      ...
      "posts_by_tag" => posts_by_tag(assigns, manifest.post_options),
      "theme" => %{
        "name" => manifest.name,
        "slug" => manifest.slug,
    ...
    inner_context =
      ...
      |> compose_post_options(theme)
      |> compose_page_options(theme)
  ...
  defp posts_by_tag(assigns, post_option_fields) do
    assigns
    |> Map.get(:all_posts, Map.get(assigns, :posts, []))
    |> Presenter.posts()
    |> Enum.map(&effective_post(&1, post_option_fields))
  ...
  defp compose_post_options(context, theme) do
    fields = theme.manifest.post_options

    context
    |> Map.replace_lazy("post", &effective_post(&1, fields))
    |> Map.replace_lazy("posts", fn posts -> Enum.map(posts, &effective_post(&1, fields)) end)
  end

  defp effective_post(post, fields) when is_map(post) do
    effective = Manifest.merge_fields(fields, Map.get(post, "post_options", %{}))
    Map.put(post, "post_options", effective)
  end

  defp effective_post(post, _fields), do: post
  ...
  defp compose_page_options(%{"page" => %{"template" => template} = page} = context, theme)
       when is_binary(template) and template != "" do
    apply_effective(context, page, page_config_fields(theme, template))
  end

  defp compose_page_options(%{"page" => %{} = page} = context, theme) do
    apply_effective(context, page, theme.manifest.page_options)
  end

  defp compose_page_options(context, _theme), do: context
  ...
  # context (so it can read `page.page_options.*`) and hand the result to the
